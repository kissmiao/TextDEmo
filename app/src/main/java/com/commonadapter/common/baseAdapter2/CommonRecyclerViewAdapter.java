package com.commonadapter.common.baseAdapter2;
...
public abstract class CommonRecyclerViewAdapter<T> extends
    ...
    public void setOnRecyclerViewItemLongClickListener(
            OnRecyclerViewItemLongClickListener clickListener) {
     ...
     */
    @Override
    public CommonRecyclerViewHolder onCreateViewHolder(ViewGroup parent,
                                                       int viewType) {
        View view = null;
        if (viewType == RecyclerView.INVALID_TYPE && mCurrentPosition != -1
                && mCurrentPosition < headers.size()) { // 说明是头部
    ...
    public void onBindViewHolder(CommonRecyclerViewHolder h, int position) {
        ...
        if (position < size) {
            ...
            if (isNeedSetClickListener(position)) { // 如果需要设置监听
                h.itemView
                        .setOnClickListener(new MyItemClickListenerAdapter(h));
                h.itemView
                        .setOnLongClickListener(new MyLongOnClick(h, position));
            }
            if (viewIdsInItem != null && viewIdsInItem.length > 0
                    && isNeedSetViewInItemClickListener(position)) {
                for (int i = 0; i < viewIdsInItem.length; i++) {
                    View v = h.getView(viewIdsInItem[i]);
                    if (v != null) {
                        v.setOnClickListener(new MyViewInItemClickListenerAdapter(
                                h, viewIdsInItem[i]));
                        v.setOnLongClickListener(new MyLongOnClick(h,
                                viewIdsInItem[i]));
                    }
                }
            }
    ...
    private boolean isNeedSetViewInItemClickListener(int position) {
        if (itemTypesForViewInItem == null
                || itemTypesForViewInItem.length == 0) {
            return true;
        }
        int itemType = getItemType(position);
     ...
     *            当前的下标
     */
    public abstract void convert(CommonRecyclerViewHolder h, T entity,
                                 int position);
     ...
     */
    public void setOnViewInItemClickListener(
            OnViewInItemClickListener onViewInItemClickListener,
            int... viewIdsInItem) {
    ...
    private class MyItemClickListenerAdapter implements View.OnClickListener {
        ...
        public void onClick(View v) {
            if (onRecyclerViewItemClickListener != null) {
                // 回调方法
                onRecyclerViewItemClickListener.onItemClick(v, h.getPosition()
                        - headers.size());
            }
        }
     ...
     */
    private class MyViewInItemClickListenerAdapter implements
        ...
        public MyViewInItemClickListenerAdapter(CommonRecyclerViewHolder h,
                                                int viewId) {
            this.h = h;
            this.viewId = viewId;
        }
        ...
        public void onClick(View v) {
            if (onViewInItemClickListener != null) {
                // 回调方法
                onViewInItemClickListener.onViewInItemClick(v, h.getPosition()
                        - headers.size());
            }
        }
    ...
    private class MyLongOnClick implements OnLongClickListener {
        ...
        public boolean onLongClick(View v) {
            if (onRecyclerViewItemLongClickListener != null) {
                onRecyclerViewItemLongClickListener.onItemLongClick(v,
                        h.getPosition());
            }
            return false;
        }
